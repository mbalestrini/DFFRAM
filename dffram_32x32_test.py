...
def openlane(*args_tuple):
    ...
    run_docker("efabless/openlane:mpw-3a", args)
...
def place_pins(design, synth_info, in_file, out_file, pin_order_file):
    ...
    if os.getenv("USE_IOPLACE") != "1":
        ...
        print("---  use io_place.py ", pin_order_file , "---")
...
def pdngen(width, height, in_file, out_file):
    ...
    # pitch = 25 # temp: till we arrive at a function that takes in width
    # offset = 0 # temp: till we arrive at a function that takes in width
    pdn_cfg = f"""
...
def obs_routes(layers, wmargin, hmargin, width, height, in_file, out_file):
    global last_def
    print("--- Routing Obstruction Creation---")
    obst = ""
    for l in layers:
        if(obst == ""):
            obst = f"met{l} 0 0 {2*wmargin+width} {2*hmargin+height}"
        else:
            obst = obst + f",met{l} 0 0 {2*wmargin+width} {2*hmargin+height}"
...
def obs_route(metal_layer, wmargin, hmargin, width, height, in_file, out_file):
    global last_def
    print("--- Routing Obstruction Creation---")
    openlane(
        "openroad", "-python",
...
def route(synth_info, in_file, out_file):
    ...
    with open(f"{build_folder}/route.tcl", 'w') as f:
        ...
        global_route \\
            ...
            -allow_congestion
        set_thread_count {os.getenv("ROUTING_CORES") or '12'}
        detailed_route \\
...
def gds(design, def_file, gds_file):
    ...
    with open(f"{build_folder}/gds.sh", "w") as f:
        ...
        export MAGIC_WRITE_FULL_LEF=0
        cat /openlane/scripts/magic/lef.tcl > ./lef.tcl
        sed -i "s/load \$::env(magic_result_file_tag).mag/load .\/magic\/{design}.mag /" ./lef.tcl
        

        echo "Streaming out GDSII..."
        magic -rcfile {pdk_magic_dir}/{pdk}.magicrc -noconsole -dnull < ./gds.tcl
        echo "Writing LEF from GDSII..."
        magic -rcfile {pdk_magic_dir}/{pdk}.magicrc -noconsole -dnull < ./lef.tcl
...
def flow(frm, to, only, pdk_root, skip, size, building_blocks, clock_period, halo, variant, drc, image, klayout, output_dir):
    ...
    steps = [
        ...
        (
            "obs_route",
            lambda: obs_routes([4,5], wmargin, hmargin, width, height, pdn, obstructed) 
        ),
        (
            "routing",
            lambda: (
